...
import { getOneBlog, updateOneBlog } from "../../../redux/actions/blogActions";
...
function AddBlogMovie() {
  ...
  const submitForm = async (values) => {
    console.log(values);
    dispatch(updateOneBlog(blogId.id, values));
  ...
  const validate = (values) => {
    ...
    if (!values.name) {
      ...
      errors.name = "! Tên blog không vượt quá 200 ký tự";
    }
    ...
    } else if (values.topImage.length > 500) {
      errors.topImage = "! Đường dẫn ảnh không vượt quá 500 ký tự";
    }
    if (!values.mainImage) {
      errors.mainImage = "! Vui lòng nhập đường dẫn ảnh";
    } else if (values.mainImage.length > 500) {
      errors.mainImage = "! Đường dẫn ảnh không vượt quá 500 ký tự";
    }
    return errors;
  };
